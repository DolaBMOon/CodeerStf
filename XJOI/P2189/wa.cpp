...
namespace Bing
{
	...
	double Calc(double l,double m,double r,double fl,double fm,double fr,double w,int dep)
	{
		double lm=(l+m)*0.5,rm=(r+m)*0.5;
		double flm=F(lm),frm=F(rm),wl,wr;
		if(abs(w-((wl=Simpson(l,m,fl,flm,fm))+(wr=Simpson(m,r,fm,frm,fr))))<eps)
			return wl+wr;
		return Calc(l,lm,m,fl,flm,fm,wl,dep+1)+Calc(m,rm,r,fm,frm,fr,wr,dep+1);
	}
	...
	double Work(Circle *_q,int _n,double L,double R)
	{
		...
		return Calc(L,m,R,a,b,c,Simpson(L,R,a,b,c),0);
	}
